# 🗺️ WORKSPACE_INDEX.md — Master Workspace Index

> **Single-page master index covering ALL systems in the workspace.** Start here. Click through to each system's detailed index for deep reference.

**Generated:** 2026-06-28
**Systems documented:** 17 | **System indexes:** 19 | **Total files tracked:** 380+ organized

---

## 🚨 START HERE

| If you need to... | Open this |
|---|---|
| Launch a client business TODAY | `MASTER_INDEX_1PAGE.md` (AusAI Tech) |
| See all AI coding tools | `AI_TOOLS_DASHBOARD.html` |
| See AI Army live dashboard | `http://localhost:8001` 🟢 |
| Find any agent (of 2,793) | `AGENT_REGISTRY_SYSTEM_INDEX.md` |
| Generate AI music videos | `ComfyUI/COMFYUI_SYSTEM_INDEX.md` |
| Harvest YouTube transcripts | `YOUTUBE_TOOLS_SYSTEM_INDEX.md` |
| Automate workflows | `N8N_AUTOMATION_SYSTEM_INDEX.md` |
| Dispatch agent squads | `AI_ARMY_SYSTEM_INDEX.md` |
| Voice-control your system | `VOICE_PA_SYSTEM_INDEX.md` |
| Sync projects across ecosystem | `AETHER_CORE_SYSTEM_INDEX.md` |
| Manage your empire dashboard | `EMPIRE_SYSTEM_INDEX.md` |
| Query your knowledge base | `ARCHON_V2_SYSTEM_INDEX.md` |
| Ingest knowledge into brain | `PROJECT_BRAIN_2_0_SYSTEM_INDEX.md` |
| Browse the ecosystem catalog | `MASTER_ECOSYSTEM_INDEX.md` |
| Run dashboard automation | `DASHBOARD_SYSTEM_INDEX.md` |
| Manage your bookmarks | `BOOKMARK_MANAGER_PRO_INDEX.md` |
| Generate AI influencer content | `AI_INFLUENCER_INDEX.md` |
| See what AI tools are installed | `AI_TOOLS_INVENTORY_INDEX.md` |
| Organize top-level clutter | `WORKSPACE_ORGANIZATION_PLAN.md` |

---

## 📊 ALL SYSTEMS — AT A GLANCE

| # | System | Files | Index | Status |
|---|---|---|---|---|
| 1 | **AusAI Tech** | 80 | `MASTER_INDEX_1PAGE.md` | ✅ 15 rounds, 67 new |
| 2 | **Agent Registry** | 13 | `AGENT_REGISTRY_SYSTEM_INDEX.md` | ✅ 2,793 agents, 16 domains |
| 3 | **AI Army (Foot Clan)** | 16 | `AI_ARMY_SYSTEM_INDEX.md` | 🟢 LIVE :8001 |
| 4 | **ComfyUI Music Video Studio** | 77k+ | `ComfyUI/COMFYUI_SYSTEM_INDEX.md` | ✅ 22-option launcher |
| 5 | **Voice PA System** | 8 | `VOICE_PA_SYSTEM_INDEX.md` | ✅ Voice-controlled assistant |
| 6 | **Aether Core System** | 24k+ | `AETHER_CORE_SYSTEM_INDEX.md` | ✅ Sync bridge, empire audit |
| 7 | **Empire Command Center** | 15+ | `EMPIRE_SYSTEM_INDEX.md` | ⚠️ Encoding fix needed |
| 8 | **Archon V2** | 50+ | `ARCHON_V2_SYSTEM_INDEX.md` | ✅ 4 microservices, 5 MCP tools |
| 9 | **Project Brain 2.0** | 10+ | `PROJECT_BRAIN_2_0_SYSTEM_INDEX.md` | ✅ SHA256 ingestion, Phase F-H |
| 10 | **Master Ecosystem Catalog** | 11+ | `MASTER_ECOSYSTEM_INDEX.md` | ✅ 11 subsystems hub |
| 11 | **Dashboard System Scripts** | 61 | `DASHBOARD_SYSTEM_INDEX.md` | ✅ Terminal automation |
| 12 | **Bookmark Manager Pro** | 7,685 | `BOOKMARK_MANAGER_PRO_INDEX.md` | ✅ 17 categories, 100% coverage |
| 13 | **AI Influencer Factory** | 4 | `AI_INFLUENCER_INDEX.md` | ✅ Ollama→ComfyUI→n8n |
| 14 | **AI Tools Inventory** | 10+ | `AI_TOOLS_INVENTORY_INDEX.md` | ✅ 5 AI platforms, 4 runtimes |
| 15 | **n8n Automation** | 95+ | `N8N_AUTOMATION_SYSTEM_INDEX.md` | ✅ Docker, 91 templates |
| 16 | **YouTube Tools** | 5 | `YOUTUBE_TOOLS_SYSTEM_INDEX.md` | ✅ Transcript harvester |
| 17 | **Workspace Meta** | 19 | (this file) | ✅ All 19 system indexes |

### Supporting Documents

| Document | Purpose |
|---|---|
| `GRAND_SUMMARY.md` | One-page condensed reference — print this |
| `CHANGELOG.md` | Complete development history across all systems |
| `WORKSPACE_ORGANIZATION_PLAN.md` | Non-destructive plan for 380-file organization (6 batches done) |
| `AI_TOOLS_DASHBOARD.html` | Unified dashboard for 14+ installed AI coding tools |
| `SKILLS_MATRIX.md` | Cross-tool skills: 13 tools × 12 skill categories |
| `AGENCY_MISSION_CONTROL.md` | 3-division command center: Dev, Content, Influencer |
| `GOLDEN_RULES.md` | Universal rules: append, preserve, protect, enhance |
| `AI_AGENT_INVENTORY.md` | 10 capability clusters + 6×6 similarity matrix |

---

## 🔄 KEY WORKFLOWS

### Find a Client → Get Paid (AusAI Tech)
```
DAY1_EXECUTION_PACK.md → Discovery_Call_Script.md → Proposal_Template.html → Invoice_Template.html → Revenue_Dashboard_Static.html
```

### Generate an AI Music Video (ComfyUI)
```
launch_music_video_studio.bat → brainstorm (OpenRouter) → transcribe (Whisper) → analyze-audio (librosa) → ComfyUI workflow → output
```

### Dispatch an Agent Squad (AI Army) 🟢 LIVE
```
python FOOTCLAN_EXECUTOR.py --task "audit revenue" → registry scoring → dispatch log → execute → report
     API: http://localhost:8001/agents → recon-001 + revenue-001 idle
```

### AI Influencer Content Pipeline
```
ai_influencer_factory.py → Ollama/qwen2.5 → Piper TTS → ComfyUI → n8n → publish
```

### Empire Full Boot
```
EMPIRE_JARVIS_LAUNCHER.bat → Dashboard (3142) + HUD (8888) + Media API (5000) + Revenue + Web3 + Content + Voice
```

---

## 🖥️ PORTS & SERVICES

| Port | Service | System | Status |
|---|---|---|---|
| 11434 | Ollama API | AI Tools | — |
| 1234 | LM Studio | AI Tools | — |
| 3142 | Empire Dashboard | Empire | ⚠️ |
| 3737 | Archon Frontend | Archon V2 | — |
| 5000 | Empire Media API | Empire | — |
| 5678 | n8n | n8n Automation | — |
| 8001 | AI Army FastAPI | AI Army | 🟢 LIVE |
| 8051 | MCP Server | Archon V2 | — |
| 8052 | Agents Service | Archon V2 | — |
| 8181 | Archon Main Server | Archon V2 | — |
| 8188 | ComfyUI | ComfyUI | — |
| 8888 | Empire HUD | Empire | — |

---

## 🔒 GOLDEN RULES (enforced across all systems)

```
⭐ Rule #1  NOTHING IS OBSOLETE
⭐ Rule #2  ALL PROJECTS ARE PERMANENT
⭐ Rule #3  ALL AI TOOLS ARE ESSENTIAL
⭐ Rule #7  ENHANCEMENT NOT REDUCTION
⭐ Rule #8  PERSONAL FOLDERS PROTECTED
⭐ Rule #9  ADD · INTEGRATE · CONNECT · DOCUMENT
```

---

## 📦 GIT STATUS

| Metric | Value |
|---|---|
| Commits on master | 88 |
| Commits this session | 16 |
| Unpushed commits | 16 (needs GitHub auth) |
| Remote | `woodsai69rme/ausai-live-site` |
| 🟢 Live | AI Army on port 8001 |

> **To push:** Create PAT at `github.com/settings/tokens` → paste token as password when pushing

---

*This is the master index. Each system has its own detailed index — click through above. Everything is append-only. Nothing is deleted.*
